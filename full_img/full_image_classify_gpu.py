...
execfile('full_image_cnn_gpu.py')
...
param_path = 'D:/dev/Rhoana/membrane_cnn/results/good2/'
param_files = glob.glob(param_path + "*.h5")

RECORD_ALL_LAYERS = False

for param_file in param_files:

    output_path = param_file.replace('.h5', '_gpu')
    ...
    for image_index in range(classify_start, classify_start + classify_n):
        ...
        for layeri in range(len(all_layers)):
            if layeri == 0:
                layer_out = all_layers[layeri].apply_layer(layer0_in)
            else:
                layer_out = all_layers[layeri].apply_layer(d_input_image=layer_out)

            if layeri == len(all_layers) - 1:
                layer_output.append(layer_out)
            elif RECORD_ALL_LAYERS:
                layer_output.append(layer_out.get())
        ...
        output_image(layer0_in[0,:,:], 99, 0, 0)
